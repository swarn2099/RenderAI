...
dotenv.config();
const BACKEND_BASE_URL = process.env.BACKEND_BASE_URL || "http://localhost:3000/api/v1/news";
const MCP_PORT = process.env.MCP_PORT || 3001;
// Add this helper function at the top (after imports):
function sanitizeKeys(obj) {
    const validKey = /^[a-zA-Z0-9_-]{1,64}$/;
    return Object.fromEntries(Object.entries(obj).filter(([key]) => validKey.test(key)));
}
...
function registerTool(name, description, parameters, handler) {
    ...
    const camelCaseParameters = Object.fromEntries(Object.entries(parameters).map(([key, value]) => {
    ...
    console.error(`[MCP] Registering tool ${name} with schema:`, JSON.stringify(claudeSchema, null, 2));
...
registerTool("get-top-headlines", "Get top news headlines", {
    ...
    console.error("[MCP] get-top-headlines params (with defaults):", fullParams);
    try {
        ...
        console.error("[MCP] get-top-headlines error:", error?.response?.data || error.message);
        throw error;
    }
});
// Tool: Search Everything
registerTool("search-news", "Search all news articles", {
    q: z.string().optional().describe("Search query"),
    sources: z.string().optional().describe("Comma-separated list of sources"),
    ...
    try {
        ...
        console.error("[MCP] search-news error:", error?.response?.data || error.message);
        throw error;
    }
});
// Tool: Get Sources
registerTool("get-sources", "Get news sources", {
    ...
    try {
        ...
        console.error("[MCP] get-sources error:", error?.response?.data || error.message);
        throw error;
    }
});
// Tool: Get Category Breakdown
registerTool("get-category-breakdown", "Get a breakdown of news articles by category", {
    chart_type: z
        .string()
        .optional()
    ...
    try {
        ...
        const errObj = error;
        console.error("[MCP] get-category-breakdown error:", errObj?.response?.data || errObj?.message || String(error));
        throw error;
    }
});
...
app.get("/tools", (req, res) => {
    ...
    console.error("[MCP] Registered tools:", JSON.stringify(registeredTools, null, 2));
...
async function main() {
    ...
    app.listen(MCP_PORT, () => {
        console.error(`MCP Tool Discovery server running on http://localhost:${MCP_PORT}`);
    });
    // Connect MCP server
